...
class TestGroupController:
    ...
    def test_get_groups(self, db, test_group):
        ...
        for i in range(3):
            group = Group(groupName=f"Group {i}", groupDescription=f"Description {i}")
            db.add(group)
        db.commit()
    ...
    def test_get_groups_with_pagination(self, db, test_group):
        ...
        for i in range(5):
            group = Group(
                groupName=f"Paginated Group {i}", groupDescription=f"Description {i}"
            )
            db.add(group)
        db.commit()
    ...
    def test_update_group(self, db, test_group):
        ...
        update_data = GroupUpdate(
            groupName="Updated Group", groupDescription="Updated description"
        )

        updated_group = GroupController.update_group(
            db, test_group.groupId, update_data
        )
    ...
    def test_add_user_to_group(self, db, test_group, test_user):
        ...
        result = GroupController.add_user_to_group(
            db, test_group.groupId, test_user.userId
        )
    ...
    def test_remove_user_from_group(self, db, test_group, test_user):
        ...
        # Then remove user
        result = GroupController.remove_user_from_group(
            db, test_group.groupId, test_user.userId
        )
...
class TestGroupEndpoints:
    ...
    def test_update_group_not_found(self, client):
        """Test updating non-existent group."""
        response = client.put("/v1/group/99999", json={"groupName": "Updated"})
    ...
    def test_add_user_to_group_endpoint(self, client, test_group, test_user):
        ...
        response = client.post(
            f"/v1/group/{test_group.groupId}/users", json={"userId": test_user.userId}
        )
    ...
    def test_remove_user_from_group_endpoint(self, client, test_group, test_user, db):
        ...
        response = client.delete(
            f"/v1/group/{test_group.groupId}/users/{test_user.userId}"
        )
    ...
    def test_create_group_missing_required_fields(self, client):
        ...
        response = client.post("/v1/group", json={})
